import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { requireUser } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { formatCurrency, formatDateTime, statusLabel } from "@/lib/utils";

export const dynamic = "force-dynamic";

export default async function AccountDonationsPage() {
  const user = await requireUser(["DONOR", "ADMIN", "PARTNER"]);
  const donations = await prisma.donation.findMany({
    where: { donorId: user.id },
    include: { campaign: true, receipt: true },
    orderBy: { createdAt: "desc" }
  });

  return (
    <div className="page-shell">
      <Card className="p-6">
        <h1 className="text-3xl font-semibold">Lịch sử ủng hộ và biên nhận</h1>
        <p className="section-lead">Mỗi giao dịch demo đều có mã giao dịch, biên nhận và trạng thái đối soát.</p>
        <div className="mt-6 table-wrap">
          <table className="w-full min-w-[800px] text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500">
              <tr>
                <th className="px-4 py-3">Thời gian</th>
                <th className="px-4 py-3">Chiến dịch</th>
                <th className="px-4 py-3">Số tiền</th>
                <th className="px-4 py-3">Trạng thái</th>
                <th className="px-4 py-3">Biên nhận</th>
                <th className="px-4 py-3">Đối soát</th>
              </tr>
            </thead>
            <tbody>
              {donations.map((donation) => (
                <tr key={donation.id} className="border-t">
                  <td className="px-4 py-3">{formatDateTime(donation.createdAt)}</td>
                  <td className="px-4 py-3 font-medium">{donation.campaign.title}</td>
                  <td className="px-4 py-3 font-semibold text-primary">{formatCurrency(donation.amount)}</td>
                  <td className="px-4 py-3"><Badge tone="success">{statusLabel(donation.status)}</Badge></td>
                  <td className="px-4 py-3">{donation.receipt?.receiptNumber}</td>
                  <td className="px-4 py-3 text-slate-500">{donation.receipt?.reconciliation}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </Card>
    </div>
  );
}
